 ...
 *
 * @param pkg The package for which to get entry points, e.g., 'core'.
 * @returns An array of secondary entry-points names, e.g., ['auth', 'translations', ...]
 */
export function getSecondaryEntryPointsForPackage(pkg: BuildPackage) {
  ...
  // E.g., extract "auth" from "from '@ajf/core/auth';".
...
function buildPackageImportStatementFindCommand(searchDirectory: string, packageName: string) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
      args: ['/r', `from.'@ajf/${packageName}/.*'`, `${searchDirectory}\\*.ts`]
    };
  } else {
    return {
      binary: 'grep',
      args: ['-Eroh', '--include', '*.ts', `from '@ajf/${packageName}/.+';`, searchDirectory]
    };
  }
}
