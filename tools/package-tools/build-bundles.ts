...
export class PackageBundler {
  ...
  /** Bundles the primary entry-point w/ given entry file, e.g. @ajf/core */
  ...
  private async _createRollupBundle(config: RollupBundleConfig) {
    ...
    const writeOptions = {
      name: config.moduleName || 'ajf.material',
    ...
    if (config.format === 'umd') {
      ...
      if (this._buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        const importRegex = new RegExp(`@ajf/${this._buildPackage.name}/.+`);
        external = external.filter(e => !importRegex.test(e));

        // Use the rollup-alias plugin to map imports of the form `@ajf/core/auth`
  ...
  /**
   * Gets mapping of import aliases (e.g. `@ajf/core/auth`) to the path of the es5
   * bundle output.
  ...
  private _getResolvedSecondaryEntryPointImportPaths(bundleOutputDir: string) {
    return this._buildPackage.secondaryEntryPoints.reduce((map, p) => {
      map[`@ajf/${this._buildPackage.name}/${p}`] =
