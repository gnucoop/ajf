...
// `ajf-docs-example` attribute which can be used to locate the divs and initialize the example
// viewer.
const EXAMPLE_PATTERN = /<!--\W*example\(([^)]+)\)\W*-->/g;

// Markdown files can contain links to other markdown files.
// Most of those links don't work in the Ajf docs, because the paths are invalid in the
...
/** Generates html files from the markdown overviews and guides for the core. */
task('markdown-docs-core', () => {
  return src(['src/core/**/!(README).md'])
      .pipe(rename({prefix: 'core-'}))
      .pipe(markdown(markdownOptions))
      .pipe(transform(transformMarkdownFiles))
      .pipe(dom(createTagNameAliaser('docs-markdown')))
      .pipe(dest('dist/docs/markdown'));
...
function fixMarkdownDocLinks(link: string, filePath: string): string {
  ...
  // guides can be loaded in the Ajf docs.
